function hFig = plot_wds_4( ...
 ...
 dx_max=max(s.nodes.X)-min(s.nodes.X);
    dy_max=max(s.nodes.Y)-min(s.nodes.Y);
    offset = 0.01*max(dx_max,dy_max);
...
% -------------------------------------------------------------------------
if add_nodes == 1
    scatter(s.nodes.X, s.nodes.Y, 60, 'ko');
end

if add_node_labels == 1
    for k = 1:numel(s.nodes.ID)
        text( s.nodes.X(k)-offset/2, ...
            s.nodes.Y(k)+offset/2, ...
